...
for each_file in csv_path:
    if len(schema):
        df = spark.read.csv(path=each_file, schema=schema,
                            header=is_header, sep=separator)
    else:
        df = spark.read.csv(path=each_file, inferSchema=True,
                            header=is_header, sep=separator)
    ...
    df.write.parquet(path=output_folder, mode=mode,
                     partitionBy=partitionBy, compression=compression)
