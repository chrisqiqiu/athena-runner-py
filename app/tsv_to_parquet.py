...
class S3Csv2ParquetConversionError(Exception):
    ...
    def __init__(self, reason):
        Exception.__init__(
            self, ' S3Csv2ParquetConversion failed: reason {}'.format(reason))
        self.reason = reason
...
class S3Csv2Parquet(TaskQueue):
    ...
    def __init__(self, region, template_location, max_jobs=5,
        ...
        super(S3Csv2Parquet, self).__init__(
            max_jobs, retry_limit, max_tasks_same_name=3)
    ...
    def _trigger_task(self, task):
        ...
        try:
            ...
            logging.error(
                "Failed to create/update job due to  {}".format(e.args[0]))
            raise e
        """
        ...
        while True:
            try:
                ...
                logger.info(
                    "Execution of Job {} has started".format(task.name))
                task.is_active = True
                break
            except self.glue.exceptions.ConcurrentRunsExceededException:
                logger.info(
                    "Attempt to run job raised ConcurrentRunsExceededException")
    ...
    def _upload_template(self):
        ...
        if ("Contents" not in key_list.keys()) or \
            ...
            local_template_path = path.join(path.dirname(
                path.abspath(__file__)), "template_scripts")
            self.s3_client.put_folder(
                local_template_path, s3_bucket, template_path)
    ...
    def _validate_parameter(csv_path, output_folder, schema,
        ...
        if not isinstance(output_folder, str) and output_folder is not None:
            logging.error("output_folder must be of type string got {} instead".format(
        ...
        if not isinstance(schema, list):
            logging.error(
                "schema parameter must be of format '[('column1', 'datatype')]'")
            raise S3Csv2ParquetConversionError(
                "schema parameter must be of format '[('column1', 'datatype')]'")

        supported_datatypes = ['boolean', 'double', 'float',
                               'integer', 'long', 'null', 'short', 'string']
        ...
        if allocated_capacity > 100 or allocated_capacity < 2:
            ...
            raise S3Csv2ParquetConversionError(
                "allocated capacity should be in range 2-100")
